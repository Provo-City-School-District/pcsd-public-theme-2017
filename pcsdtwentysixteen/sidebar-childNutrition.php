<aside id="sidebar" class="cnSidebar sidebar">
		<?php
		$i_am_buttons = curl_init();
		curl_setopt($i_am_buttons, CURLOPT_URL, 'https://globalassets.provo.edu/globalpages/i-am-sidebar-links.php');
		curl_setopt($i_am_buttons, CURLOPT_HEADER, 0);
			// grab URL and pass it to the browser
			curl_exec($i_am_buttons);
			// close cURL resource, and free up system resources
			curl_close($i_am_buttons);
		?>

		<ul class="cn-bos">
			<li><img src="https://provo.edu/wp-content/uploads/2017/01/best-in-state.png" alt="" /><span><em>Multiple winner of Best of State</em></span></li>
		</ul>
		<ul class="imagelist">
			<!--
			<li>
				<a href="http://itsmealsprovo.com/index.php?page=whatsnew&sid=2302081511134871">
					<img src="//globalassets.provo.edu/image/icons/cart-lt.svg" alt="" />
					<span>Prices & payment Options</span>
				</a>
			</li>
			-->
			<li>
				<a href="https://docs.google.com/forms/d/e/1FAIpQLScxAhg6X4sFgnANbpCW1o5JyV_gfyEKjHBAvnrkaITdqmh65Q/viewform">
					<img src="//globalassets.provo.edu/image/icons/form-lt.svg" alt="" />
					<span>School Meals Order Form</span>
				</a>
			</li>
			<li>
				<a href="https://provo.edu/wp-content/uploads/2020/08/Policy-3401-Wellness.pdf">
					<img src="//globalassets.provo.edu/image/icons/policies-lt.svg" alt="" />
					<span>3401 Wellness</span>
				</a>
			</li>
			<li>
				<a href="https://provo.edu/wp-content/uploads/2020/08/3401-P1-Wellness-Vending-Machine-Guidelines.pdf">
					<img src="//globalassets.provo.edu/image/icons/policies-lt.svg" alt="" />
					<span>3401 Procedure 1 Wellness: Vending Machine Guidelines</span>
				</a>
			</li>
			<li>
				<a href="https://provo.edu/wp-content/uploads/2020/08/3401-P2-Wellness-Physical-Activity-Guidelines.pdf">
					<img src="//globalassets.provo.edu/image/icons/policies-lt.svg" alt="" />
					<span>3401 Procedure 2 Wellness: Physical Activity Guidelines</span>
				</a>
			</li>
			<li>
				<a href="https://provo.edu/wp-content/uploads/2019/05/05-21-2019-wellness-policy-triennial-assessment.pdf">
					<img src="https://globalassets.provo.edu/image/icons/policies-lt.svg" alt="" />
					<span>Wellness Policy Triennial Assessment</span>
				</a>
			</li>
			<li>
				<a href="https://provo.edu/category/news/child-nutrition/">
					<img src="//globalassets.provo.edu/image/icons/news-white.svg" alt="" />
					<span>Child Nutrition News</span>
				</a>
			</li>
		</ul>
		<ul class="sociallinks">
			<li><a href="https://www.facebook.com/ITSMealsProvo/"><img src="//globalassets.provo.edu/image/icons/facebook.svg" alt="Link to Facebook" /></a></li>
			<li><a href=""><img src="//globalassets.provo.edu/image/icons/instagram.svg" alt="Link to Instagram" /></a></li>
			<li><a href=""><img src="//globalassets.provo.edu/image/icons/twitter.svg" alt="Link to Twitter" /></a></li>
		</ul>
	<h2>Child Nutrition Contacts</h2>
		<?php  
			echo file_get_contents('https://provo.edu/directory_page/child-nutrition/');
		?>
</aside>